## ObAddon Ambient Sounds Pack

This Doom launcher addon activates ambient sounds in ObAddon prefabs ("sound" branch)
(https://github.com/caligari87/ObAddon/tree/sound)

To use this addon:

1. Download or clone the repo as desired.
2. Add the download to your Doom launcher (e.g. GZDoom)
	
For more information about ObAddon and all available addons, join the OBLIGE discord!
A link to the Discord and more information about ObAddon is available at:
(https://github.com/caligari87/ObAddon)

Special thanks to Scionox for helping to make this sound pack possible!
